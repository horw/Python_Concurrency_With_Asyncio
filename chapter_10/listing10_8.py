...
async def all_products(request: Request) -> Response:
    async with aiohttp.ClientSession() as session:
        ...
        if products in pending:
            ...
            [request.cansel() for request in requests]
            logging.exception('Server error reaching product service.', exc_info=products.exception())

            return web.json_response({'error': 'CServer error reaching product service.'},
                                     status=500)
        else:
            product_response = await products.result().json()
            product_result: List[Dict] = await get_products_with_inventory(session, product_response)
