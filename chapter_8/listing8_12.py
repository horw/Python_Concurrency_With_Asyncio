...
class ServerState:
    ...
    async def _echo(self, reader: StreamReader, writer: StreamWriter):
        try:
            ...
            logging.exception('Error reading from client.', exc_info=0)
            self._writers.remove(writer)
...
async def main():
    ...
    async def client_connection(reader: StreamReader, writer: StreamWriter) -> None:
        await server_state.add_client(reader, writer)

    server = await asyncio.start_server(client_connection, '127.0.0.1',8000)
