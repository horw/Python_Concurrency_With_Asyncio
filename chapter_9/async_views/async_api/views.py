...
async def make_requests(url:str, request_num: int):
    async with aiohttp.ClientSession() as session:
        ...
        successful_result = [result for result in results if not isinstance(result, Exception)]
        return {'failed_results': failed_results, 'successful_results': successful_result}
...
async def sync_to_async_view(request):
    ...
    thread_sensitive: bool = request.GET['thread_sensitive'] == 'False'
